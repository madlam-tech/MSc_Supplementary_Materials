...
if [ -e ./BC10.$jobid.meryl/merylIndex ] ; then
...
if [ -e BC10.$jobid.meryl.tar.gz ]; then
  exist1=true
else
  exist1=false
fi
...
/scale_wlg_persistent/filesets/opt_nesi/CS400_centos7_bdw/Canu/2.2-GCC-9.2.0/bin/meryl k=22 threads=4 memory=2 \
  count \
    segment=$jobid/01 ../../BC10.seqStore \
    output ./BC10.$jobid.meryl.WORKING \
&& \
mv -f ./BC10.$jobid.meryl.WORKING ./BC10.$jobid.meryl
