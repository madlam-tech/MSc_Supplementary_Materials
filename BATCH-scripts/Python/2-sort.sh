...
#  This script should be executed from unitigging/BC10.ovlStore.BUILDING/, but the binary needs
...
if [ -e ./BC10.ovlStore.BUILDING/$jobname.info -a ! -e ./BC10.ovlStore.BUILDING/$jobname.started ] ; then
  echo "Sorting job finished; info file './BC10.ovlStore.BUILDING/$jobname.info' exists."
  exit
fi
#
#  Sort!
...
$bin/ovStoreSorter \
  -deletelate \
  -O  ./BC10.ovlStore.BUILDING \
  -S ../BC10.seqStore \
  -C  ./BC10.ovlStore.config \
  -f \
  -s $jobid \
  -M 1 
