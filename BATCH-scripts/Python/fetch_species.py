...
output_dir = "./updated_fasta_files"
...
# Update the headers of the FASTA files
def replace_header(fasta_file, new_header, output_file):
...
# Update the headers in the FASTA files
for filename in os.listdir(input_dir):
    if filename.endswith(".fasta"):
        acc = filename.split('_concatenated')[0]
        species_name = species_names.get(acc, "Unknown_species")
        if " " in species_name:
            genus, species = species_name.split()[:2]
            new_header = f"{genus[0]}.{species}"
        else:
            new_header = species_name
        input_path = os.path.join(input_dir, filename)
        output_path = os.path.join(output_dir, filename)
        replace_header(input_path, new_header, output_path)
