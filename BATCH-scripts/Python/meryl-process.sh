...
if [ -e ./BC10.ms22.histogram -a -e ./BC10.ms22.dump -a -e ./BC10.ms22.ignore.gz ] ; then
  exit 0
fi
...
if [ -e BC10.ms22.histogram ]; then
  exists1=true
else
  exists1=false
fi

if [ -e BC10.ms22.dump ]; then
  exists2=true
else
  exists2=false
fi

if [ -e BC10.ms22.ignore.gz ]; then
  exists3=true
else
  exists3=false
fi
if [ $exists1 = true -a $exists2 = true -a $exists3 = true ] ; then
  echo "Output files 'BC10.ms22.histogram', 'BC10.ms22.dump' and 'BC10.ms22.ignore.gz' exist in 'unitigging/0-mercounts'."
  exit 0
fi
...
if [ ! -e ./BC10.ms22/merylIndex ] ; then
  /scale_wlg_persistent/filesets/opt_nesi/CS400_centos7_bdw/Canu/2.2-GCC-9.2.0/bin/meryl threads=4 memory=2 \
    greater-than 1 \
      output BC10.ms22.WORKING \
      union-sum  \
        ./BC10.01.meryl \
  && \
  mv -f ./BC10.ms22.WORKING ./BC10.ms22
  ...
  rm -rf ./BC10.01.meryl ./BC10.01.meryl.err
fi
...
if [ ! -e ./BC10.ms22.histogram ] ; then
  /scale_wlg_persistent/filesets/opt_nesi/CS400_centos7_bdw/Canu/2.2-GCC-9.2.0/bin/meryl threads=1 memory=1 \
    statistics ./BC10.ms22 \
  > ./BC10.ms22.histogram.WORKING \
  && \
  mv ./BC10.ms22.histogram.WORKING ./BC10.ms22.histogram
fi
...
if [ ! -e ./BC10.ms22.dump ] ; then
  /scale_wlg_persistent/filesets/opt_nesi/CS400_centos7_bdw/Canu/2.2-GCC-9.2.0/bin/meryl threads=4 memory=2 \
    print ./BC10.ms22.##.dump \
      at-least distinct=0.9999 \
        ./BC10.ms22

  cat ./BC10.ms22.??.dump > ./BC10.ms22.dump
  rm -f ./BC10.ms22.??.dump
fi
...
if [ ! -e ./BC10.ms22.ignore.gz ] ; then
  numKmers=`wc -l < ./BC10.ms22.dump`
  totKmers=`/scale_wlg_persistent/filesets/opt_nesi/CS400_centos7_bdw/Canu/2.2-GCC-9.2.0/bin/meryl statistics ./BC10.ms22 | grep present | awk '{ print $2 }'`


  ./meryl-make-ignore.pl $numKmers $totKmers < ./BC10.ms22.dump | gzip -1c > ./BC10.ms22.ignore.gz
fi
